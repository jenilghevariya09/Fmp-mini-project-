...
module.exports.controller = function (app, io, socket_list) {
    ...
    app.post('/api/admin/stock_transfer', (req, res) => {
        helper.Dlog(req.body)
        var reqObj = req.body;
        helper.CheckParameterValid(res, reqObj, ['access_token'], () => {
            db.query('INSERT INTO `stock_transfer_log` (`user_id`,`id`,`Branch_name`,`Stock_name`,`Stock_quantity`) VALUES (?,?,?,?,?)', [reqObj.user_id,reqObj.id,reqObj.Branch_name,reqObj.Stock_name,reqObj.Stock_quantity], (err, result) => {
                if (err) {
    ...
    app.post('/api/admin/stock_transferlist', (req, res) => {
        helper.Dlog(req.body)
        var reqObj = req.body;
        helper.CheckParameterValid(res, reqObj, ['access_token', 'user_id'], () => {
            db.query('select * from stock_transfer_log', [], (err, result) => {
    ...
    app.post('/api/admin/stock_transferupdate/:id', (req, res) => {
    helper.Dlog(req.body)
    var reqObj = req.body;
    var id = req.params.id;
    helper.CheckParameterValid(res, reqObj, ['access_token'], () => {
        db.query('UPDATE stock_transfer_log SET Branch_name=?,Stock_name=? , Stock_quantity=?    WHERE id =?;', [reqObj.Branch_name, reqObj.Stock_name,reqObj.Stock_quantity,id], (err, result) => {
...
app.post('/api/admin/stock_transferdelete/:id', (req, res) => {
    helper.Dlog(req.body)
    var reqObj = req.body;
    var id = req.params.id;
    helper.CheckParameterValid(res, reqObj, ['access_token'], () => {
        db.query('DELETE FROM stock_transfer_log WHERE id =?;', [id], (err, result) => {
