...
module.exports.controller = function(app, io, socket_list) {
    ...
    app.post('/api/admin/login', (req, res) => {
        ...
        helper.CheckParameterValid(res, reqObj, ['email', 'password', 'socket_id'], () => {
            ...
            debugger
            db.query('UPDATE `user_detail_admin` SET `access_token`= ?   WHERE `email` = ? AND `password` = ? AND `user_type`= 1', [access_token, reqObj.email, reqObj.password], (err, result) => {
                if (err) {
                ...
                if (result['affectedRows'] == 1) {
                    ...
                    db.query('SELECT `user_id`,`image_name`, `name`, `email`, `mobile`, `access_token` FROM `user_detail_admin` WHERE `email` = ?', [reqObj.email], (err, result) => {
                        if (err) {
                            helper.ThrowHtmlError(err, res);
                            return
                        }
            ...
            helper.CheckParameterValid(res, fields, ['access_token', "user_id"], () => {
                ...
                helper.CheckParameterValid(res, files, ["image"], () => {
                    ...
                    fs.copyFile(files.image[0].path, new_path, () => {
                        ...
                        db.query("UPDATE `user_detail_admin` SET `image_name` = ? WHERE `user_id` = ?", [image_name, fields.user_id[0]], (err, result) => {
                            if (err) {
                                helper.ThrowHtmlError(err, res)
                                return
                            }
    ...
    app.post('/api/admin/user_list_admin', (req, res) => {
        helper.Dlog(req.body);
        var reqObj = req.body;

        helper.CheckParameterValid(res, reqObj, ['user_id', 'access_token'], () => {
            db.query('SELECT * FROM `user_detail_admin` ', [], (err, result) => {
                if (err) {
                    helper.ThrowHtmlError(err, res);
                    return;
                }
